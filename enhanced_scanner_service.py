...
from ..config.api_config import APISectionManager, ScanSection
...
class EnhancedScannerService:
    ...
    async def run_comprehensive_scan(self, target_url: str) -> Dict[str, Any]:
        try:
            ...
            # Stage 4: Parallel Vulnerability Assessment
            ...
            await self.db_manager.save_scan_results(target_url, result)
    ...
    async def _run_section_scan(self, section: ScanSection, target_url: str,
                              executor: ThreadPoolExecutor) -> Dict[str, Any]:
        try:
            api_key = self.api_manager.get_next_available_key(section)
            if not api_key:
                return {
                    'status': 'failed',
                    'error': 'No available API keys for this section',
                    'vulnerabilities': []
                }

            scanner = self._get_scanner(section)
            if not scanner:
                return {
                    'status': 'failed',
            ...
            # Run the scan in the thread pool
            loop = asyncio.get_event_loop()
            vulnerabilities = await loop.run_in_executor(
                executor,
                lambda: scanner.scan(target_url)
            )
            ...
            print(f"Error in {section.value} scan: {str(e)}")
            return {
                'status': 'failed',
                'error': str(e),
                'vulnerabilities': []
    ...
    def _get_scanner(self, section: ScanSection) -> Optional[Any]:
        scanners = {
            ScanSection.OWASP: EnhancedOWASPScanner,
            ScanSection.ZERODAY: EnhancedZeroDayAI,
            ScanSection.CVE: EnhancedCVEScanner,
            ScanSection.DATABASE: EnhancedDatabaseScanner
        }
        
        scanner_class = scanners.get(section)
        if scanner_class:
            return scanner_class()
        return None

    async def get_scan_results(self, target_url: str) -> List[Dict[str, Any]]:
        try:
            results = []
            for section in ScanSection:
                section_results = await self.db_manager.get_section_results(
                    target_url,
                    section.value
                )
                if section_results:
                    results.append({
                        ...
                        'timestamp': section_results.get('timestamp', time.time())
                    })
            return results
        except Exception as e:
